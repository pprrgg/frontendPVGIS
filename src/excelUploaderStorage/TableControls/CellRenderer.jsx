...
// ==========================
// POLYGON EDITOR - VERSIÓN CORREGIDA
// ==========================


const PolygonEditorOpcionNoUsadaquefunciona = ({ polygon, setPolygon, saveValue, keyName }) => {
  ...
  const createCompassRose = (center) => {
    ...
    const CompassIcon = L.divIcon({
      className: "compass-rose-icon",
      html: `<canvas id="compassCanvas" width="${size}" height="${size}" style="background: transparent;"></canvas>`,
      iconSize: [size, size],
      iconAnchor: [size / 2, size / 2],
    });

    compassRef.current = L.marker(center, {
      icon: CompassIcon,
      pane: "compassPane",
    });
    ...
    setTimeout(() => {
      ...
      const textRadius = Math.min(radius + padding, size / 2 - 10);
      const radioLength = textRadius - 10;

      for (let deg = -180; deg <= 180; deg += 10) {
        const rad = ((deg + 90) * Math.PI) / 180;
        const x = cx + radioLength * Math.cos(rad);
        const y = cy + radioLength * Math.sin(rad);

        ctx.lineWidth = 0.5;
        ctx.setLineDash(deg % 20 === 0 ? [] : [4, 4]);
      ...
      for (let deg = -180; deg <= 180; deg += 20) {
        const rad = ((deg + 90) * Math.PI) / 180;
        const labelX = cx + textRadius * Math.cos(rad);
        const labelY = cy + textRadius * Math.sin(rad);

        let label = "";
        if (deg === 0) label = "S";
        else if (deg === 180 || deg === -180) label = "N";
        else if (deg === -90) label = "E";
        else if (deg === 90) label = "O";
  ...
  const scaleCoordinates = (coords) => {
    ...
    return coords.map((p) => [
      centerLat + (p[0] - centerLat) * SCALE_FACTOR,
      centerLng + (p[1] - centerLng) * SCALE_FACTOR,
    ]);
  };

  const unscaleCoordinates = (coords) => {
    if (!coords.length) return [];
    const centerLat = coords.reduce((sum, p) => sum + p[0], 0) / coords.length;
    const centerLng = coords.reduce((sum, p) => sum + p[1], 0) / coords.length;
    return coords.map((p) => [
  ...
  const handleMapReady = (map) => {
    ...
    if (!map.getPane("compassPane")) {
      map.createPane("compassPane");
      map.getPane("compassPane").style.zIndex = 200;
      map.getPane("compassPane").style.pointerEvents = "none";
    }
  ...
  useEffect(() => {
    ...
    if (polygon.length > 0) {
      const scaledPolygon = scaleCoordinates(polygon);
      const poly = L.polygon(scaledPolygon, { color: "#ff0000" });
      drawnItems.addLayer(poly);

      const center = getPolygonCenter(scaledPolygon);
      const compassRose = createCompassRose(center);
      compassRose.addTo(map);

      map.fitBounds(poly.getBounds(), { padding: [120, 120], maxZoom: 22 });
    }
  }, [polygon]);
  ...
  useEffect(() => {
    ...
    const updatePolygon = (layer) => {
      ...
      const unscaled = unscaleCoordinates(latlngs);
      setPolygon(unscaled);
      saveValue(JSON.stringify({ [keyName]: [unscaled] }));
    };
    ...
    const onCreated = (e) => {
      ...
      if (compassRef.current) map.removeLayer(compassRef.current);
      const center = getPolygonCenter(
        e.layer.getLatLngs()[0].map((p) => [p.lat, p.lng])
      );
      const compassRose = createCompassRose(center);
      compassRose.addTo(map);

      map.fitBounds(e.layer.getBounds(), { padding: [120, 120], maxZoom: 22 });
    };
    ...
    const onEdited = (e) => {
      ...
      if (bounds.isValid())
        map.fitBounds(bounds, { padding: [120, 120], maxZoom: 22 });
    };
  ...
  return (
    <div
      style={{
        width: "180%", // Aumentamos tamaño real del contenedor
        height: 560,
        position: "relative",
      }}
    >
      ...
      >
        <style>{`
          ...
          .leaflet-interactive { stroke-width: 3px !important; }
          .compass-rose-icon { background: transparent !important; border: none !important; }
        `}</style>
      </MapContainer>
    </div>
  );
...
const PolygonEditorbueno = ({ polygon, setPolygon, saveValue, keyName }) => {
  const mapRef = useRef(null);
  const drawnItemsRef = useRef(null);
  const compassRef = useRef(null);

  // ELIMINAR: SCALE_FACTOR innecesario - las coordenadas deben mantenerse reales
  const DEFAULT_CENTER = [36.66417, -4.45865];
  const DEFAULT_ZOOM = 19;

  // Crear la rosa de los vientos en un pane separado
  const createCompassRose = (center) => {
    const map = mapRef.current;
    if (!map) return null;
    ...
    // Crear marker en el pane de la rosa
    compassRef.current = L.marker(center, {
      icon: CompassIcon,
      pane: "compassPane",
    });
  ...
  // ELIMINAR: funciones de escalado innecesarias
  // Las coordenadas deben mantenerse en su tamaño real

  const handleMapReady = (map) => {
    mapRef.current = map;

    // Crear pane para la rosa
    if (!map.getPane("compassPane")) {
      map.createPane("compassPane");
      map.getPane("compassPane").style.zIndex = 200; // detrás de polígonos (400)
      map.getPane("compassPane").style.pointerEvents = "none"; // no bloquear clicks
    }
  ...
  // Renderizar el polígono y la rosa al cambiar polygon
  useEffect(() => {
    const map = mapRef.current;
    const drawnItems = drawnItemsRef.current;
    if (!map || !drawnItems) return;
    ...
    if (polygon.length > 0) {
      // USAR COORDENADAS REALES SIN ESCALAR
  ...
  // Manejar creación, edición y borrado
  useEffect(() => {
    const map = mapRef.current;
    if (!map) return;
    const drawnItems = drawnItemsRef.current;
    ...
    const updatePolygon = (layer) => {
      const latlngs = layer.getLatLngs()[0].map((p) => [p.lat, p.lng]);
      // USAR COORDENADAS REALES DIRECTAMENTE
      setPolygon(latlngs);
  ...
  return (
    <div
      style={{
        ...
        height: 560,
        position: "relative",
        // ELIMINAR: transform: scale(1.8) - esto causa el problema principal
        // Si necesitas hacer zoom, usa el zoom del mapa (DEFAULT_ZOOM)
      }}
    >
      <MapContainer
        center={DEFAULT_CENTER}
